...
class I2CDevice:
    ...
    def readinto(self, buf, start=0, end=None):
        ...
        self.__readfrom_into(self.device_address, buf, start=start, end=end)
    ...
    def write(self, buf, start=0, end=None, stop=True):
        ...
        self.__writeto(self.device_address, buf, start=start, end=end, stop=stop)
    ...
    def __probe_for_device(self) -> None:
        ...
        """
        try:
            self.__writeto(self.device_address, b"")
        except OSError:
            # some OS's dont like writing an empty bytesting...
            # Retry by reading a byte
            try:
                result = bytearray(1)
                self.__readfrom_into(self.device_address, result)
            except OSError:
                # pylint: disable=raise-missing-from
                raise ValueError("No I2C device at address: 0x%x" % self.device_address)
                # pylint: enable=raise-missing-from

    # @cajomferro: from Adarfuit_blinka.busio.py
    def __readfrom_into(self, address, buffer, start=0, end=None):
    ...
    # @cajomferro: from Adarfuit_blinka.busio.py
    def __writeto(self, address, buffer, start=0, end=None, stop=True):
    ...
    # @cajomferro: from Adarfuit_blinka.busio.py
    def __writeto_then_readfrom(self, address, buffer_out, buffer_in, out_start=0, out_end=None, in_start=0,
        ...
        """
        self.__writeto(address, buffer_out, start=out_start, end=out_end, stop=stop)
        return self.__readfrom_into(address, buffer_in, start=in_start, end=in_end)
